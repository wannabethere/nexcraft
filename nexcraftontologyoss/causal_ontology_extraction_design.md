...
Operators that compute pairwise and multivariate dependencies across columns
or features.

| Correlator        | Method                              | Use case                                        |
| ----------------- | ----------------------------------- | ----------------------------------------------- |
| `LinearCorrelator`| Pearson                             | Numeric ↔ numeric, linear relationships         |
| `RankCorrelator`  | Spearman, Kendall                   | Numeric ↔ numeric, monotonic non-linear         |
| `MixedCorrelator` | phik, correlation ratio η           | Mixed numeric and categorical                   |
| `CategoricalCorrelator` | Cramér's V, Theil's U         | Categorical ↔ categorical                       |
| `MICorrelator`    | Mutual information                  | Any ↔ any, non-parametric                       |
| `DistanceCorrelator` | Distance correlation              | Non-linear, rotation-invariant                  |
| `TimeSeriesCorrelator` | Cross-correlation, transfer entropy | Time-series with potential lag relationships  |

Correlators emit one finding per *significant* pair, where significance is
governed by configurable thresholds — typically a minimum effect size (e.g.,
|r| > 0.1) and a maximum p-value or false-discovery-rate-corrected significance.
Insignificant pairs are dropped at write time. This is the discipline from §11.2
of the ingestion plan; it keeps the findings store from drowning under
combinatorial pair counts.

For correlators on wide tables, the framework supports **column subsetting**:
either user-specified columns of interest or automatic selection based on the
profiler's output (skip columns with > 90% null, > 99% same value, etc.).

### 3.4 Causal Structure Discoverers

A separate construct from correlators because the methods, the cost profile,
and the output structure are different. Correlators emit pairwise findings;
causal discoverers emit candidate DAG edges.
...
| GES         | Greedy equivalence search; cleaner for purely Gaussian | `causal-learn` |
| LiNGAM      | Non-Gaussian noise assumption holds             | `lingam`        |
| DirectLiNGAM | Faster LiNGAM variant                          | `lingam`        |
| NOTEARS     | Continuous, differentiable, neural-priors-friendly | `notears`     |
| Granger     | Time-series with explicit temporal ordering     | `statsmodels`   |
...
intersects them: edges that appear in 3+ algorithms with consistent direction
are flagged "high agreement"; disagreement edges are flagged "needs review."
The Causal Hypothesizer in card generation reads both — high-agreement edges
become hypothesized causal_edge cards with elevated initial confidence;
disagreement edges become hypothesized cards with lowered confidence and a
prose note about the algorithmic disagreement.

Causal discovery is expensive — PC on a 200-node graph can take hours. The
framework supports:

- **Variable subsetting** based on prior interest (only run on columns flagged
  as decision-relevant).
- **Schema-bounded discovery**: don't propose edges across tables that have
  no foreign-key relationship and no semantic affinity.
- **Tiered cadence**: full discovery monthly, incremental updates daily on
  the subset of variables that changed.

### 3.5 Document Chunkers
