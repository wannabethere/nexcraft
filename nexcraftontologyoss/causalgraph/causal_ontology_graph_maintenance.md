...
Used by: causal effect computation, counterfactual simulation, attribution
queries, identifiability checking.
...
1. **One-hop neighborhood fetch.** "Give me everything connected to
   ...
   during retrieval.
2. **Causal subgraph for a target node.** "Give me every edge into
   ComplianceGap with weight > 0.2." Used by attribution queries.
3. **Permission filter.** "Can role X see object_type Y?" Used on every
   query.
4. **Lineage trace.** "Walk back from this derived value to source rows."
   Used in EXPLAIN queries.
5. **Concept ancestors.** "What parent concepts does PhishingRisk roll up
...
| Causal subgraph          | Redis, keyed by target node + filter | On any causal_edge weight update      |
...
A nightly job refreshes these. Query traffic skips the live graph entirely
for these patterns.
