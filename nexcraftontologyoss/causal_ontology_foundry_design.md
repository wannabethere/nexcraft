...
Operators that require sample data. Each has a degradation strategy when
...
Per the extraction design §3.3: Pearson, Spearman, Cramér's V, phik,
mutual information, distance correlation. With Mode B:

- Sample-size lower bounds are enforced. Correlations on fewer than 100
  rows are not emitted at all.
- Significance thresholds are tightened in proportion to sample size to
  control false-discovery.
- Confidence intervals via bootstrap are computed and emitted alongside
  point estimates.

In Mode C, the operator skips entirely.
...
priors first, and they always emit findings that downstream card
generation can compose into the existing causal scaffold.
...
graph with literature-backed edges keyed to seed concepts.

#### Causal Discovery Suite

Per the extraction design §3.4: PC, FGES, GES, LiNGAM, NOTEARS, PCMCI.
Multiple algorithms run in parallel on tenant sample data; consensus
operator intersects outputs.

The Foundry adjustment: **discovered edges are reconciled with prior
edges**. When discovery finds an edge between X and Y, the operator checks
whether a prior edge exists. Three cases:
...
In Mode C, this operator skips entirely. Mode B uses algorithms tolerant
of small samples (PC and LiNGAM tolerate; FGES needs more data).
