...
### 6.5 Depth-aware faithfulness

Causal queries default to depth 3 (per design §6.2) and may extend further
on user request. Depth introduces failure modes that simpler causal evals
don't catch.

**Edge-existence at depth.** Every claim in a response that traverses
N hops must be grounded in N actual `causal_edge` cards along a real path.
Audit: extract claims from response, parse the implied path, verify each
hop exists in the graph. A claim referencing a depth-3 path that uses a
non-existent middle edge is a hallucination of the path, not of any
individual edge — caught only by path-level checking.

**Compounding-uncertainty communication.** At depth ≥2, the response must
explicitly communicate that the magnitude estimate has compounded
uncertainty. A response that presents a depth-3 path estimate with the
same confidence framing as a direct edge fails this check. The judge
verifies that responses traversing depth ≥2 include language like "less
precise than direct causes" or "compounded uncertainty" or quantitative
hedging ("the estimate is between X and Y").

**Boundary-honest expansion offers.** When the response synthesizer reaches
the depth-3 boundary, it surfaces expansion offers for the boundary nodes.
The eval verifies these offers are accurate — they reference real boundary
nodes (not invented ones), and the "additional causes available" counts
match the actual graph state.

**Path Shapley correctness.** When attribution is reported across multiple
paths to the same target, the Shapley contributions must sum to 100% within
floating-point tolerance. A response claiming "60% from path A, 50% from
path B" is mathematically incoherent. The eval checks the arithmetic.

**Depth-3 vs deeper consistency.** When a user explicitly requests depth ≥4
via `DEPTH N` or `CAUSAL PATH FROM X TO Y`, the response should be
consistent with what depth-3 said about the overlapping portion. A
response that contradicts its own prior depth-3 claims as the user expands
