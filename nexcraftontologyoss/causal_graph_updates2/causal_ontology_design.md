...
# DEPTH defaults to 3 if unspecified
...
# Path query — bounded by path length, not by depth
CAUSAL PATH FROM CourseDesign TO ComplianceGap
MAX_LENGTH 5

# Progressive expansion — extends a prior reasoning context
EXPAND CAUSE TimeManagement DEPTH 3
FROM PRIOR_QUERY <prior_id>
...
**Bounded reasoning depth.** Causal queries default to depth 3 — three hops
from the target node. This matches what humans can hold in working memory
(cause → mediator → effect), keeps query latency predictable, and bounds
compounding uncertainty. Direct causal_edge cards are stored authoritatively;
multi-hop paths are computed on demand from those direct edges, which means
no risk of stored multi-hop edges drifting from their constituents. Users
expand beyond depth 3 by explicit `DEPTH N` or by progressive expansion via
`EXPAND CAUSE`.

**Compounded uncertainty at depth.** Each additional causal hop multiplies
uncertainty. A direct edge with weight 0.6 ± 0.05 becomes a depth-3 path
with weight ~0.22 with substantially wider CI. Responses that traverse to
depth 3 explicitly communicate this — *"the magnitude estimate is less
precise than for direct causes"* — rather than presenting compounded
estimates as if they had direct-edge precision.

**Progressive expansion.** When the response synthesizer reaches depth 3,
it identifies boundary nodes (those at the depth-3 frontier) and surfaces
them as expansion offers. The user signals deeper interest by either
naming a boundary node or asking a follow-up that requires nodes beyond
the boundary. The planner then runs a fresh depth-3 traversal from the
named boundary, composing the new subgraph with what was already in
context.
...
4. **LLM-free math layer.** Statistical correlation, causal discovery,
   weight estimation, identifiability checking, and confidence interval
   computation never use LLMs. These are mechanical operations on data,
   reproducible and auditable. LLMs propose hypotheses (candidate
   confounders, candidate stratifications) and frame results in prose, but
   the math itself is owned by scipy, statsmodels, causal-learn, and dowhy.
   This separation is what makes the causal layer trustworthy.
5. **Bounded reasoning depth.** Causal queries default to depth 3 — three
   hops from the target. Stored authoritatively: direct causal_edge cards.
   Computed on demand: multi-hop paths and their compounded uncertainty.
   Users expand depth explicitly when the default is insufficient. This
   keeps query latency predictable and prevents stored multi-hop edges
   from drifting from their constituents.
6. **Provenance-traceable.** Every value, weight, and inference points back
   to the rows or cards that produced it.
7. **Readable by default.** If a card is unreadable, it is broken. Prose is
   the contract.
